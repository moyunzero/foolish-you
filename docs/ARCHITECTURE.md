...
graph TD
  ...
  subgraph storage["lib/storage/"]
    ...
    Migrate[snapshotMigration / snapshotPrep]
  end
...
4. If missing or stale date: **`buildNewDailySnapshot()`** calls **`selectDailyGame()`** with `dateKey`, optional `previous` (to avoid repeating yesterday’s type/hash), and dev-only `forceGameType`. Writes a new `DailySnapshot` with `status: 'playing'`.
...
3. Router sends user to **`app/result.tsx`** (via index redirect or game navigation).
...
- Persistence uses **`@react-native-async-storage/async-storage`** only; no API routes or sync in v1.
...
| `@foolish-you/streak-v1` | `StreakState`: `currentStreak`, `lastCheckInDateKey` |
...
- **New game type**: extend `GameType`, add generator under `lib/puzzles/`, branch in `dailySelector` and grid components; keep determinism via `deriveSubSeed`.
