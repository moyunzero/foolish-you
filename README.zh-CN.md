...
**iOS App Store：** 线上 [`2.5.0`](https://apps.apple.com/app/id6770218110)（内容加深 + Adaptive Mastery）。Android Google Play 尚未发布。
...
`app.json` 中已配置应用名 **傻了么**、Bundle ID `com.moyunzero.foolish-you`、深色界面。营销版本 **`2.5.0`** 已在 App Store 上架（EAS `production`）；留存 KPI 见 App Store Connect Analytics（应用内无第三方 analytics SDK）。
...
| **当前 v2.5（`2.5.0` 已上架）** | 待 ASC | 待 ASC | 待 ASC | 目标 ≥ 4.5 | 目标 ≥ 5% |
...
| **v2.4** | **已上架**（`2.4.1`） | ✅ 周日特辑 + ✅ 数绘扩充；✅ Composition 版式；内容加深（周节奏 + 数绘 120）已随 `2.5.0` 上架 | 周日约会感（D7）+ 补内容短板 + 主持感版式 |
| **v2.5** | **已上架**（`2.5.0`） | ✅ 内容加深（周节奏 + 数绘 120）+ ✅ Adaptive Mastery（分题型掌握度档 + 本机去重）；第五玩法仍挂起 | 无徽章的个人难度适配 · 诚实披露未玩日期可能变盘 |
