...
**iOS App Store：** 线上 [`2.4.1`](https://apps.apple.com/app/id6770218110)；待提交 **`2.4.2`**（内容加深：周节奏 + 数绘 120）。Android Google Play 尚未发布。
...
`app.json` 中已配置应用名 **傻了么**、Bundle ID `com.moyunzero.foolish-you`、深色界面。营销版本 **`2.4.2`** 经 EAS `production` 构建提交 App Store Connect（审核通过前线上仍为 `2.4.1`）；留存 KPI 见 App Store Connect Analytics（应用内无第三方 analytics SDK）。
...
| **当前 v2.4（`2.4.2` 待提交 / `2.4.1` 已上架）** | 待 ASC | 待 ASC | 待 ASC | 目标 ≥ 4.5 | 目标 ≥ 5% |
...
| **v2.4** | **已上架**（`2.4.1`）· **`2.4.2` 待提交**（内容加深） | ✅ 周日特辑 + ✅ 数绘扩充；✅ Composition 版式；✅ 周节奏难度带 + 数绘库至 120；第五玩法推迟到 v2.5 | 周日约会感（D7）+ 补内容短板 + 主持感版式 + 内容加深 |
