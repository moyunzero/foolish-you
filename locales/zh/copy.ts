...
export const gameRules = {
  ...
  nonogram: {
    title: '数绘规则',
    intro: '在 8×8 格子上，根据每行、每列的数字提示涂格，拼出完整图案。',
    bullets: [
      '每个数字表示该行/列中一段连续填色块，顺序与提示从左到右、从上到下一致',
      '一行/列有多个数字时，表示多段填色块，段与段之间至少隔一个空格',
      '点格子在 空 → 填色 → 标记(×) 之间切换，长按清空',
      '× 仅作辅助标记，不参与完成判定',
      '过程中不会提示对错；该填的格子全部填对后，点「完成今日」',
    ],
  },
  slitherlink: {
    title: '数回规则',
    intro:
      '在 7×7 点阵上沿网格边画线，连成一条且仅一条闭合回路；有数字的格子表示其四边中有几条边属于回路，无数字的格子不受限制。',
    bullets: [
      '轻点某条边：未标 → 连线 → ×（排除），按此顺序循环',
      '长按某条边：清除为未标（无连线、无 ×）',
      '数字为 0～3，表示该格四边中有几条边画在回路上',
      '只能有一条闭合回路，不可交叉或分叉；完成时每个点连 0 条或恰好 2 条线',
      '全部数字与回路一致后，点「完成今日」',
    ],
  },
} as const;
