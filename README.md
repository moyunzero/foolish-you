...
| **v1.2** 系统语言 | 跟随设备 `zh` / `en`（英文品牌 **Silly Me**）；双语隐私政策；**无**正式版应用内语言设置 |
...
| **v1.2** | **当前（`1.2.0`）** | 系统语言 zh/en（`expo-localization`）；英文品牌 **Silly Me**；`locales/` + `useI18n`；双语隐私；DevTools **设置占位**（不写存储，Release 无入口） | 海外可读性 + 商店合规 |
