...
当前覆盖：日期工具、RNG、每日选题与 safe 选题、数独/二进制/数绘生成与校验、存储迁移/恢复、完成历史、战报与统计、评分门槛、连签、Context 与主要屏幕 RTL（**272** 项）。另含 `npm run test:migration` 迁移黄金样例。UI 动效与真机布局以手测为主。
