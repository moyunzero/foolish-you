...
│   ├── game.tsx            # 游戏主界面（数独 / 二进制 / 数绘 / 数回分支）
│   ├── result.tsx          # 完成 / 认怂结果页（数绘/数回含揭示卡）
│   ├── privacy.tsx         # 应用内隐私政策
...
│   └── result/             # 结果页徽章、统计卡、战报按钮、数绘/数回揭示卡与动效
├── contexts/
...
│   ├── puzzles/            # 数独、二进制、数绘、数回、每日选题、可解性、RNG
...
当前覆盖：日期工具、RNG、每日选题与 safe 选题、数独/二进制/数绘/数回生成与校验、存储迁移/恢复、完成历史、战报与统计、评分门槛、连签、i18n（含 `en-smoke`）、Context 与主要屏幕 RTL（**386** 项）。另含 `npm run test:migration` 迁移黄金样例。UI 动效与真机布局以手测为主。
